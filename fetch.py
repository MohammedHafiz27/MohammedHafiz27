...
FONT_FILE_BITMAP = os.path.join(
    os.path.dirname(gifos.__file__),
    "fonts",
    "gohufont-uni-14.pil",
)
...
def main():
    ...
    ).strftime("%a %b %d %I:%M:%S %p %Z %Y")

    t.gen_text(
        "GIF_OS Modular BIOS v1.0.11",
        1,
    )

    t.gen_text(
        f"Copyright (C) {year_now}, \x1b[31mHafiz Softwares Inc.\x1b[0m",
        2,
    )

    t.gen_text(
        "\x1b[94mGitHub Profile ReadMe Terminal, Rev 1011\x1b[0m",
        4,
    )

    t.gen_text(
        "Flutter(tm) GIFCPU - 250Hz",
        6,
    )
    ...
    t.gen_text(
        "\x1b[93mHafiz OS v1.0.11 (tty1)\x1b[0m",
        1,
        count=5,
    )
    ...
    git_user_details = gifos.utils.fetch_github_stats(
        GITHUB_USERNAME,
        IGNORE_REPOS,
    )
    ...
    top_languages = [
        lang[0]
        for lang in git_user_details.languages_sorted
    ]
